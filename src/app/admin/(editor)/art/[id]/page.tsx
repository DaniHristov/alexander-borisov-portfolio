...
export default async function EditArtPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const data = await getWorkingProjectById(id).catch(() => null);
  if (!data || data.project.gallery !== 'art') notFound();
  ...
  return (
    <section>
      <div className="mb-4 flex items-center gap-3">
        <Link href="/admin/art" className="text-xs text-neutral-400 hover:text-white">← Back to grid</Link>
        <h1 className="text-sm uppercase tracking-wide text-neutral-400">Edit artwork</h1>
      </div>
